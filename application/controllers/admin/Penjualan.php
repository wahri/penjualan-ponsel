...
class Penjualan extends Admin_Controller
{
    ...
    public function index()
    {
		...
		if (isset($_POST['filter_button'])) {
			...
			redirect('admin/penjualan/exportCSV/'.$sproses.'/'.$stopik);
		...
		$this->load->view('admin/penjualan/view/index', $this->data);
    }
	...
	public function add($id = NULL){
		...
		if($cek_data->num_rows() > 0){
			...
			if ($this->form_validation->run() === TRUE) {
				...
				$this->load->view('admin/penjualan/add/index', $this->data);
			}
		}else{
	...
	public function pesanan_baru()
    {
		...
		$config['base_url'] 			= site_url('admin/penjualan/pesanan_baru');
		$config['full_tag_open'] 		= '<ul class="pagination">';
		$config['full_tag_close'] 		= '</ul>';
		$config['prev_link'] 			= '&lt;';
		$config['prev_tag_open'] 		= '<li>';
		...
		$this->load->view('admin/penjualan/pesanan_baru/index', $this->data);
    }
	...
	public function pesanan_selesai()
    {
		...
		$config['base_url'] 			= site_url('admin/penjualan/pesanan_selesai');
		$config['full_tag_open'] 		= '<ul class="pagination">';
		$config['full_tag_close'] 		= '</ul>';
		$config['prev_link'] 			= '&lt;';
		$config['prev_tag_open'] 		= '<li>';
		...
		$this->load->view('admin/penjualan/pesanan_selesai/index', $this->data);
    }
	...
	public function pesanan_batal()
    {
		...
		$config['base_url'] 			= site_url('admin/penjualan/pesanan_batal');
		$config['full_tag_open'] 		= '<ul class="pagination">';
		$config['full_tag_close'] 		= '</ul>';
		$config['prev_link'] 			= '&lt;';
		$config['prev_tag_open'] 		= '<li>';
		...
		$this->load->view('admin/penjualan/pesanan_batal/index', $this->data);
    }
	...
	public function inv($id = NULL){
		...
		if($cek_data->num_rows() > 0){
			...
			$this->load->view('admin/penjualan/view/inv', $this->data);
		}else{
	...
	public function view($id = NULL){
		...
		if($cek_data->num_rows() > 0){
			...
			$this->data['pelanggan']		= $this->admin_penjualan_model->get_table('my_pelanggan', array('id'=> $inv->pelanggan_id))->row();
			$this->data['produk']			= $this->admin_penjualan_model->get_table('my_penjualan_item', array('no_transaksi'=> $inv->no_transaksi))->result();
			$this->load->view('admin/penjualan/view/view', $this->data);
    ...
    public function search()
    {
		...
		if($search_status = 'all'){
			...
			$view = $this->load->view('admin/penjualan/lib/list_baru', $this->data);
			return $view;
			...
			if($search_status = 'proses'){
				$view = $this->load->view('admin/penjualan/pesanan_baru/list_baru', $this->data);
			}else if($search_status = 'selesai'){
				$view = $this->load->view('admin/penjualan/pesanan_selesai/list_baru', $this->data);
			}else {
				$view = $this->load->view('admin/penjualan/pesanan_batal/list_baru', $this->data);
			}
			return $view;
		}
    }
    ...
    public function search_by_status()
    {
        ...
        $view = $this->load->view('admin/penjualan/lib/list_page', $this->data);
        return $view;
