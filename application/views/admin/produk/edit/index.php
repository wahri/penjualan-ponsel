...
<form class="form-horizontal form-label-left" action="<?php echo url_web('admin/produk/edit_produk'); ?>" method="post" enctype="multipart/form-data">
					...
					<div class="form-group">
						...
						<div class="col-md-3">
							<select class="form-control" name="merek" id="merek">
								...
								?>
										<option value="<?php echo $merek_ven->id; ?>" <?php if($merek_select == $merek_ven->id){echo "selected";} ?> ><?php echo $merek_ven->merek; ?></option>
								<?php
									}
								?>
							</select>
					...
					<div class="form-group">
						...
						<div class="col-md-4">
							<select class="form-control" name="status" required>
							  ...
							  <option value="1" <?php if($status_produk == 1){echo 'selected'; } ?>>dijual</option>
							  <option value="0" <?php if($status_produk == 0){echo 'selected'; } ?>>Tidak dijual</option>
							</select>
						</div>
					</div>
					<div class="form-group">
					...
					<div class="form-group">
						...
						<div class="col-md-4">
							<select class="form-control" name="kondisi" required>
							  <option>-Pilihan-</option>
							   <option value="Baru" <?php if($kondisi_produk == 'Baru'){echo 'selected'; } ?>>Baru</option>
							  <option value="Bekas" <?php if($kondisi_produk == 'Bekas'){echo 'selected'; } ?>>Bekas</option>
							  <option value="Rekondisi" <?php if($kondisi_produk == 'Rekondisi'){echo 'selected'; } ?>>Rekondisi</option>
							</select>
						</div>
					</div>
					<div class="form-group">
							...
							<select class="form-control" name="estalase">
							  ...
							  <?php
									if(isset($estalase))
									{
										...
										{
											?>
											<option value='<?php echo $estalase_group->id; ?>' <?php if($estalase_produk == $estalase_group->id){echo 'selected'; } ?>><?php echo $estalase_group->estalase; ?></option>
											<?php
										}
									}
								?>	
			...
			<div class="x_panel">
				...
				<div class="x_content" >
					<?php
							...
							{
								$por_id = $this->admin_produk_model->get_table('my_produk_portal', array('produk_id'=>$produk_id, 'portal_id'=>$group->id));
								if($por_id->num_rows() > 0){
										$portal_data 	= $por_id->row();
									
										?>			
											<div class="form-group">
												<label class="col-sm-2 control-label"></label>
												<div class="col-sm-4">
													<input type="text" class="form-control" name="<?php echo $group->portal; ?>" value="<?php echo $portal_data->url_portal;?>" placeholder="URL <?php echo $group->portal; ?>">
												</div>
												<div class="col-sm-4">
										...
										?>			
											<div class="form-group">
												<label class="col-sm-2 control-label"></label>
												<div class="col-sm-4">
													<input type="text" class="form-control" name="<?php echo $group->portal; ?>" "value="" placeholder="URL <?php echo $group->portal; ?>">
												</div>
												<div class="col-sm-4">
													<img src="<?php echo base_url($this->costume->get_original($group->gambar,'original')->row()->url); ?>" height="30" alt=""></img>
												</div>
				...
				<div class="x_content" >
					...
					<div class="form-group">
                        ...
                        <div class="col-md-9 col-sm-9">
							<div class="checkbox">
							  <label>
								<input type="checkbox" name="promo" value="1" <?php if (!empty($tampil_utama)) { echo "checked"; } ?>>
...
<?php 
if (!empty ($catagory_select[0])){
	$kat_1 = $catagory_select[0];
}else{
	$kat_1 = 0;
}
if (!empty ($catagory_select[1])){
	$kat_2 = $catagory_select[1];
}else{
	$kat_2 = 0;
}
if (!empty ($catagory_select[2])){
	$kat_3 = $catagory_select[2];
}else{
	$kat_3 = 0;
}
?>
...
$(document).ready(function() {
	...
	var kate_1 = <?php echo $kat_1; ?>;
	var kate_2 = <?php echo $kat_2; ?>;
	var kate_3 = <?php echo $kat_3; ?>;
	
	if(kate_2 != 0){
		$('.list_kategori_dua').hide();
		$.ajax({
			url: '<?php echo url_web('admin/produk/search_category'); ?>',
			type: 'post',
			//dataType: 'json',
			dataType: 'html',
			data: {
			...
			success: function (msg) {
				if(msg == ""){
					...
					$('#kat2').html(msg);
					$("#kat2 [value= "+kate_2+"]").attr("selected","selected");
				}
			}
		});
	}
		...
		$.ajax({
			...
			success: function (msg) {
				if(msg == ""){
					...
					$('#kat3').html(msg);
					$("#kat3 [value= "+kate_3+"]").attr("selected","selected");
				}
			}
		});
	}
