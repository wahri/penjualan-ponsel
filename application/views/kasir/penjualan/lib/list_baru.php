<?php if (!empty($penjualan_baru)) { ?>
    <?php foreach ($penjualan_baru as $lists) { ?>
        <tr class="even pointer">
			...
			<td>
				...
				<?php 
					...
					if($eks == " Waktu Habis"){
						...
						if($lists->status == 'proses'){
						?>
							<p>Batas konfirmasi <span class="label label-default"><?php echo $eks; ?></span></p>	
							<p><a href="<?php echo url_web('kasir/penjualan/transaksi/'); ?>" type="button" class="btn btn-sm btn-success">Konfirmasi Pembayaran</a></p>
						<?php
						}else if($lists->status == 'selesai'){
						?>
							<p><i class="fa fa-check-square-o"></i> Transaksi Selesai</p>
						<?php	
						}else{
						?>
							<p><i class="fa fa-times"> Transaksi dibatalkan</p>
						<?php		
						}
					}
				?>	
			...
			<td>
				<a href="<?php echo url_web('kasir/penjualan/inv/'.$lists->id); ?>" target="_blank">
					<span style="font-size:12px"><i class="fa fa-print"></i> inv</span>
				</a>
				<br>
				<a href="<?php echo url_web('kasir/penjualan/view/'.$lists->id); ?>">
